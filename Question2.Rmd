...
print(dim(svd1$u[,1, drop = FALSE]))  # Should be n x 1
print(dim(matrix(svd1$d[1], nrow = 1, ncol = 1)))  # Should be 1 x 1
print(dim(t(svd1$v[,1, drop = FALSE])))
D1<-matrix(svd1$d[1], nrow = 1, ncol = 1)
approx1<-svd1$u[,1] %*% diag(D1) %*% t(svd1$v[,1:1])
...
## 4.
To store the rank 150 approximation, you need to store the matrices U, Σ, and V^T with only the first 150 singular values/vectors. If the original image matrix is m×n:
U is m×150
Σ is 150×150
V^T is 150×n

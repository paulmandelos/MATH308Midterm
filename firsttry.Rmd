...
analogy_tasks <- read.csv('/Users/christinawei/Desktop/Math308 Midterm Project/analogy_task.txt', header = TRUE)


# Initialize a counter for correct answers
correct_answers <- 0

# Step 2: Solve Analogy Tasks
for (i in 1:nrow(analogy_tasks)) {
  ...
  idx_A <- which(dictionary == A)
  idx_B <- which(dictionary == B)
  idx_C <- which(dictionary == C)
  ...
  similarities <- matrix(NA, nrow = length(dictionary), ncol = 1)
  
  for (j in 1:length(dictionary)) {
    if (!dictionary[j] %in% c(A, B, C)) {
      similarities[j] <- sum(target_vector * V_norm[j, ])
    }
  }
  
  # Find the most similar word
  similarities[idx_A] <- -Inf  # Exclude A
  similarities[idx_B] <- -Inf  # Exclude B
  similarities[idx_C] <- -Inf  # Exclude C
